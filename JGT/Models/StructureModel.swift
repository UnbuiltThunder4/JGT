...
class Catapult: Structure {
    ...
    public func update(_ tossScene: TossScene) {
        ...
        if (self.hasRock) {
            gameLogic.spawnProjectile(tossScene, spawnPoint: CGPoint(x: self.position.x, y: self.position.y), destinationPoint: CGPoint(x: self.position.x + 250, y: self.position.y + 500), type: .rock)
            self.texture = SKTexture(imageNamed: "catapult")
            self.hasRock = false
        }
    }
